...
from .utils.analyze_winrate import analyze_winrate
from .permissions import IsOptionsPermission


class WinrateViewSet(viewsets.ModelViewSet):
    """
    ...
    def retrieve(self, request, race=None):
        ...
        winrate_data = analyze_winrate(account_replays, race)
        if winrate_data:
            serialized_data = json.dumps(winrate_data)
        else:
